...
class UnitreeSdk2Bridge:
    ...
    def PublishLowState(self, obs: Dict[str, any]):
        ...
        # NOVO: Injetando os Sensores de Pressão e Temperatura Simulados (Esquerda)
        if hasattr(self.left_hand_state, 'press_sensor_state'):
            idx = 0
            for area_idx in range(len(self.left_hand_state.press_sensor_state)):
                for sensor_idx in range(len(self.left_hand_state.press_sensor_state[area_idx].pressure)):
                    if idx < 33:
                        # Pressão vem do MuJoCo, Temperatura fixamos em 25.0 ºC (+ um micro ruído pra rede não viciar)
                        press_val = obs["left_hand_pressure"][idx]
                        temp_val = 25.0 + np.random.uniform(-0.1, 0.1)
                        
                        self.left_hand_state.press_sensor_state[area_idx].pressure[sensor_idx] = press_val
                        self.left_hand_state.press_sensor_state[area_idx].temperature[sensor_idx] = temp_val
                        idx += 1
                        
        self.left_hand_state_puber.Write(self.left_hand_state)

        # Repete a lógica para a mão direita
        for i in range(self.num_hand_motor):
            self.right_hand_state.motor_state[i].q = obs["right_hand_q"][i]
            self.right_hand_state.motor_state[i].dq = obs["right_hand_dq"][i]
            
        # NOVO: Injetando os Sensores de Pressão e Temperatura Simulados (Direita)
        if hasattr(self.right_hand_state, 'press_sensor_state'):
            idx = 0
            for area_idx in range(len(self.right_hand_state.press_sensor_state)):
                for sensor_idx in range(len(self.right_hand_state.press_sensor_state[area_idx].pressure)):
                    if idx < 33:
                        press_val = obs["right_hand_pressure"][idx]
                        temp_val = 25.0 + np.random.uniform(-0.1, 0.1)
                        
                        self.right_hand_state.press_sensor_state[area_idx].pressure[sensor_idx] = press_val
                        self.right_hand_state.press_sensor_state[area_idx].temperature[sensor_idx] = temp_val
                        idx += 1
