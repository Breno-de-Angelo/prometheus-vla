...
def main():
    ...
    xml_path = Path(__file__).parent / "unitree-g1-mujoco/assets/scene_29dof.xml"
    ...
    try:
        ...
        if hand_site_id < 0:
            print("[IK] ⚠️ Nenhum site da mão encontrado, usando corpo do braço")
            hand_body_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "right_rubber_hand")
            if hand_body_id < 0:
                print("[IK] ❌ Não encontrou right_rubber_hand")
                return
            hand_site_id = None
        else:
            print(f"[IK] ✅ Site da mão encontrado: {hand_site_name} (id={hand_site_id})")

        # Posição do copo (HARDCODED - o copo é adicionado dinamicamente no run_sim.py)
        # Você pode ajustar essas coordenadas depois de ver onde o copo realmente está
        cup_pos = np.array([0.4, -0.3, 0.8])  # X, Y, Z (ajuste conforme necessário)
    ...
    for step in range(max_steps):
        ...
        # Calcula a velocidade necessária
        try:
            jac_pinv = np.linalg.pinv(jac_arm)
            dq = jac_pinv @ (error * gain)
        except:
            print("[IK] ❌ Erro ao calcular pseudo-inversa")
            break
