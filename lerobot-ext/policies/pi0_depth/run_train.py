...
def train(cfg: CustomTrainPipelineConfig, accelerator: Accelerator | None = None):
    ...
    # ── Correção: neutraliza stats ImageNet do depth ─────────────────────────
    # O factory.py do LeRobot (linha 128) sobrescreve as stats de TODAS as
    # camera_keys com ImageNet quando use_imagenet_stats=True, incluindo o
    # depth. O NormalizerProcessorStep usa essas stats corrompidas e normaliza
    # o depth com mean=[0.485,0.456,0.406] / std=[0.229,0.224,0.225].
    # Resultado: a PointNet recebe valores negativos → zero pontos válidos →
    # token de depth constante → modelo ignora a geometria 3D.
    #
    ...
    #
    # Aplica para ACT-D (actdepth) e PI05-D (pi05depth).
    # Não modifica o LeRobot — corrigimos externamente aqui.
    ...
    if any(_DEPTH_KEY in _ds.meta.stats for _ds in _datasets_to_fix):
        logging.info(
            "[ACT-D/PI05-D] Stats do depth resetadas para identidade (mean=0, std=1). "
